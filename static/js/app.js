...
function stopSimulation() {
    ...
    fetch('/stop_simulation', { method: 'POST', headers: { 'Content-Type': 'application/json' } })
        .then(() => {
...
function updateChart(chart, time, value) {
    ...
    if (chart.options.scales?.x && time > chart.options.scales.x.suggestedMax) {
...
function updateBatteryChart(chart, nodes) {
    ...
    chart.data.datasets[0].data = [levels.high, levels.medium, levels.low, levels.critical];
    chart.update();
}
...
function updateNetworkVisualization(data) {
    ...
    const svg = d3.select("#network-canvas").selectAll("svg")
        .data([null])
        .join("svg")
        .attr("width", width)
        .attr("height", height);
